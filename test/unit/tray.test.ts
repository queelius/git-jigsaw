...
describe('Tray', () => {
  it('lists all pieces when state is empty (8x8)', () => {
    const t = new Tray(new PuzzleState(8, SEED), 'queelius', 8);
    expect(t.unplaced().length).toBe(64);
  });

  it('lists all pieces when state is empty (10x10)', () => {
    const t = new Tray(new PuzzleState(10, SEED), 'queelius', 10);
    expect(t.unplaced().length).toBe(100);
  });

  it('omits placed pieces', () => {
    const s = new PuzzleState(8, SEED);
    s.ingest([{ op: 'place', piece: 0, slot: [0, 0], rotation: 0, grid_size: 8, actor: 'a', ts: 't', v: 1, sha: 'x' }]);
  ...
  it('order is deterministic for the same actor name', () => {
    const a = new Tray(new PuzzleState(8, SEED), 'queelius', 8).unplaced();
    const b = new Tray(new PuzzleState(8, SEED), 'queelius', 8).unplaced();
    expect(a).toEqual(b);
  });

  it('order differs across actor names', () => {
    const a = new Tray(new PuzzleState(8, SEED), 'alice', 8).unplaced();
    const b = new Tray(new PuzzleState(8, SEED), 'bob', 8).unplaced();
