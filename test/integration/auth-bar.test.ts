...
describe('auth-bar', () => {
  ...
  it('shows placed-count, total, and contributors-count', () => {
    const state = new PuzzleState(8, SEED);
    state.ingest([
      { op: 'place', piece: 0, slot: [0, 0], rotation: 0, grid_size: 8, actor: 'a', ts: 't', v: 1, sha: 'x' },
      { op: 'place', piece: 1, slot: [0, 1], rotation: 0, grid_size: 8, actor: 'b', ts: 't', v: 1, sha: 'y' },
    ]);
    mountAuthBar(host, { state, store: new MockStore(), week: '2026-W17', gridSize: 8 });
  ...
  it('shows Sign in button when unauthenticated', () => {
    ...
    mountAuthBar(host, { state: new PuzzleState(8, SEED), store, week: '2026-W17', gridSize: 8 });
